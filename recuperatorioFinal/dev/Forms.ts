...
function crearCampo(key, valor) {
    ...
    switch (claseDato.tipoDato(key)) {
        ...
        case 'Color':
            ...
            inputColor.onchange = () => {
                let estilo = "background-color:" + $('#color').val();
                let t = $('#tablaAlta');
                t.attr("style", estilo);
                t = $('#tablaMod');
                t.attr("style", estilo);
...
function cargarSeleccion() {
    ...
    JSON.parse(localStorage.listado).forEach(element => {
        if (element.id == this.dato.id) {
            dato = element;
        }
    });
    ...
    for (var key in dato) {
        ...
        if (dato != null) {
            ...
            if (key == 'color') { tabla.setAttribute("style", "background-color: " + dato[key]); }
        }
        tabla.append(tr);
    }
